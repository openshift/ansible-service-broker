...
type ServiceBindingLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ServiceBinding, err error)
...
// List lists all ServiceBindings in the indexer.
func (s *serviceBindingLister) List(selector labels.Selector) (ret []*v1.ServiceBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServiceBinding))
	})
	return ret, err
}
...
type ServiceBindingNamespaceLister interface {
	...
	Get(name string) (*v1.ServiceBinding, error)
...
// List lists all ServiceBindings in the indexer for a given namespace.
func (s serviceBindingNamespaceLister) List(selector labels.Selector) (ret []*v1.ServiceBinding, err error) {
...
// Get retrieves the ServiceBinding from the indexer for a given namespace and name.
func (s serviceBindingNamespaceLister) Get(name string) (*v1.ServiceBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("servicebinding"), name)
	}
	return obj.(*v1.ServiceBinding), nil
}
