...
func (p *UpdateJob) Run(token string, msgBuffer chan<- WorkMsg) {
	podName, extCreds, err := apb.Update(p.serviceInstance, p.clusterConfig, p.log)

	if err != nil {
		p.log.Error("broker::Update error occurred.")
		p.log.Errorf("%s", err.Error())

		// send error message
		// can't have an error type in a struct you want marshalled
		// https://github.com/golang/go/issues/5161
		msgBuffer <- UpdateMsg{InstanceUUID: p.serviceInstance.ID.String(),
