...
// Update - will run the abp with the update action.
...
func Update(
	...
	if !updateProjectExists(ns) {
		log.Error("Project %s does NOT exist! Cannot update requested %s", ns, instance.Spec.FQName)
	...
	if err != nil {
		log.Error("apb::Update error occurred.")
...
func updateProjectExists(project string) bool {
