...
func Covers(ownerRules, servantRules []PolicyRule) (bool, []PolicyRule) {
...
func BreakdownRule(rule PolicyRule) []PolicyRule {
	subrules := []PolicyRule{}
	...
	for nonResourceURL := range rule.NonResourceURLs {
		for verb := range rule.Verbs {
			subrules = append(subrules, PolicyRule{Verbs: sets.NewString(verb), NonResourceURLs: sets.NewString(nonResourceURL)})
		}
	}
...
func breakdownRuleForGroup(group string, rule PolicyRule) []PolicyRule {
	subrules := []PolicyRule{}
		...
		for verb := range rule.Verbs {
			if len(rule.ResourceNames) > 0 {
				for _, resourceName := range rule.ResourceNames.List() {
					subrules = append(subrules, PolicyRule{APIGroups: []string{group}, Resources: sets.NewString(resource), Verbs: sets.NewString(verb), ResourceNames: sets.NewString(resourceName)})
				}

			} else {
				subrules = append(subrules, PolicyRule{APIGroups: []string{group}, Resources: sets.NewString(resource), Verbs: sets.NewString(verb)})
			}
		}
	}
...
func ruleCovers(ownerRule, subrule PolicyRule) bool {
	...
	groupMatches := ownerGroups.Has(APIGroupAll) || ownerGroups.HasAll(subrule.APIGroups...) || (len(ownerRule.APIGroups) == 0 && len(subrule.APIGroups) == 0)

	verbMatches := ownerRule.Verbs.Has(VerbAll) || ownerRule.Verbs.HasAll(subrule.Verbs.List()...)
	resourceMatches := ownerRule.Resources.Has(ResourceAll) || allResources.HasAll(subrule.Resources.List()...)
...
func nonResourceRuleCovers(allowedPaths sets.String, requestedPaths sets.String) bool {
	if allowedPaths.Has(NonResourceAll) {
		return true
	}
