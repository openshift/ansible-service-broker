...
| auth_type     | How the broker should read the credentials (`secret` or `file`)                                                                  |     N    |
...
Registries which implements the Docker [Registry HTTP API V2](https://docs.docker.com/registry/spec/api/) protocol will utilize the `apiv2` registry adapter. The following provides several examples.

#### OpenShift Registry

Load APBs that are published to this type of [registry](http://www.projectatomic.io/registry/).
...
Third-party images in the Red Hat Container Catalog are served from the Red Hat Connect Partner Registry (registry.connect.redhat.com). This is an authenticated repository and will require authentication credentials to access.
